...
if($_POST['action'] == 'Submit'){
        ...
        $url="login.bulksmsgateway.in/sendmessage.php?user=".urlencode($username)."&password=".urlencode($password)."&mobile=".urlencode($billing_tel)."&sender=".urlencode($sender)."&message=".urlencode($message)."&type=".urlencode('3'); 
        $ch = curl_init($url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        echo $curl_scraped_page = curl_exec($ch);
        curl_close($ch); 
    	
    	header("location:thankyou-annual_session.php");
}
?>
  <!--==========================
    Intro Section
    ...
    box-shadow: inset 1px 2px 8px rgba(0, 0, 0, 0.07);
              ...
              <div class="row">
                  <div class="col-sm-12 algncenter">
                      <input type="hidden" name="formname" value="participates_session_form"/>
